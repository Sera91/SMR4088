...
def _all_reduce_and_broadcast(strategy: Strategy, state: DistributedState) -> None:
    ...
    for layer_index, tensor, stream_context, stream_index in strategy.mapping:
        with stream_context:
            ...
            # TODO: average the weights through a PyTorch all_reduce
            # The communication primitives are contained in the PyTorch "distributed" package, here imported as dist
            # The weights to be averaged are contained in the "tensor" variable
            # Use an appropria reduce operation (many are available: dist.ReduceOp.SUM, dist.ReduceOp.SUM, etc...)
            # The process group interested by this all_reduce is contained in the "state.federated_group[stream_index]" variable
